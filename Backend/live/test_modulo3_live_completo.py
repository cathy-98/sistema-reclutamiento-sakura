...
def run():
    ...
    # Autoservicio candidato: lectura y edición segura de su propio perfil.
    own = req("GET", "/candidatos/me/perfil-completo", expected=(200,), token=cand_token).json()
    if int(own["cand_id"]) != cand_id or "cand_password" in own:
        raise LiveQAError("Perfil propio incorrecto o expuso password")
    req("PATCH", "/candidatos/me", expected=(200,), token=cand_token, json={"cand_telefono": "977776666", "cand_titulo": f"QA SELF {RUN}", "cand_url_1": ["https://linkedin.com/in/self-live", "https://github.com/self-live"]})
    req("PATCH", "/candidatos/me", expected=(422,), token=cand_token, json={"cand_email": f"hack-{RUN}@sakura.cl"})
    req("GET", f"/candidatos/{cand_id}", expected=(403,), token=cand_token)
    passed("Autoservicio perfil propio + campos protegidos + aislamiento por ID")

    # CRUD autoservicio habilidad.
    self_skill_payload = {"cdhb_habilidad_id": ctx.habilidad_id, "cdhb_anios_experiencia": 5}
    if ctx.nivel_id:
        self_skill_payload["cdhb_nivel_habilidad_id"] = ctx.nivel_id
    self_skill = req("POST", "/candidatos/me/habilidades", expected=(201,), token=cand_token, json=self_skill_payload).json()
    req("GET", "/candidatos/me/habilidades", expected=(200,), token=cand_token)
    req("PATCH", f"/candidatos/me/habilidades/{self_skill['cdhb_id']}", expected=(200,), token=cand_token, json={"cdhb_anios_experiencia": 6})
    passed("Autoservicio habilidades")
    ...
    skills_now = req("GET", f"/candidatos/{cand_id}/habilidades", expected=(200,), token=token).json()
    skill = next((x for x in skills_now if int(x.get("cdhb_habilidad_id") or 0) == ctx.habilidad_id), None)
    if not skill:
        raise LiveQAError("La habilidad creada por autoservicio no fue visible al usuario interno")
    req("POST", f"/candidatos/{cand_id}/habilidades", expected=(409,), token=token, json=self_skill_payload)
    req("PATCH", f"/candidatos/{cand_id}/habilidades/{skill['cdhb_id']}", expected=(200,), token=token, json={"cdhb_anios_experiencia": 6})
    passed("Habilidad compartida entre autoservicio y vista interna + duplicado")
    ...
    own_apps = req("GET", "/candidatos/me/solicitudes", expected=(200,), token=cand_token).json()
    if not any(int(x["slcd_id"]) == app_id for x in own_apps):
        raise LiveQAError("El candidato no ve su propia solicitud/postulación")
    req("PATCH", f"/postulaciones/{app_id}", expected=(403,), token=cand_token, json={"slcd_observaciones":"No permitido"})
    passed("Candidato ve sus postulaciones pero no puede editarlas")
